...
public class IntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task GetLessonPage_ReturnsSuccessAndContent()
    {
        // Act
        var response = await _client.GetAsync("/Lessons/1");

        // Assert
        response.EnsureSuccessStatusCode();
        var content = await response.Content.ReadAsStringAsync();
        Assert.Contains("Lesson", content, System.StringComparison.OrdinalIgnoreCase); 
    }
}
